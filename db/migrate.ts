...
if (!adminExists) {
  ...
  sqlite.prepare(`INSERT INTO utilisateurs (nom, email, mot_de_passe, role, actif, created_at) VALUES (?, ?, ?, 'admin', 1, datetime('now'))`).run('Administrateur', 'admin@facturestock.ma', hash)
  console.log('✅ Utilisateur admin créé : admin@facturestock.ma / admin123')
}

console.log('✅ Base de données initialisée avec succès (v2)')
sqlite.close()
